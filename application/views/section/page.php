...
<div class="wrapper">
  ...
  <aside class="main-sidebar sidebar-dark-primary elevation-4">
    <a href="javascript:void(0)" onclick="home()" class="brand-link">
      <i class="fas fa-university ml-3 mr-2"></i>
      <span class="brand-text font-weight-bold text-white" style="font-family: sans-serif;"><?= TITLE ?></span>
    </a>
    <div class="sidebar">
      <div class="user-panel mt-3 pb-3 mb-3 d-flex">
        <div class="image">
          <img src="<?= base_url(IMAGE . $this->library->image($session['image'])) ?>" class="img-circle elevation-1" alt="User Image">
        </div>
        <div class="info">
          <a type="button" onclick="profile()" class="d-block" title="Profile"><?= ucwords($session['full_name'] )?></a>
        </div>
      </div>
      <?php if ($this->session->userdata('role_id') == 1): ?>
        <?php $this->load->view('section/sidebar');  ?>
      <?php elseif ($this->session->userdata('role_id') == 2): ?>
        <?php $this->load->view('section/sidebar_teacher');  ?>
      <?php elseif ($this->session->userdata('role_id') == 3): ?>
        <?php $this->load->view('section/sidebar_student');  ?>
      <?php endif ?>
    </div>
  </aside>
